# -*- coding: utf-8 -*-
"""
Created on Mon Jan 15 14:35:23 2018

@author: Administrator
"""

# BCC_option_valuation.py
...
def BCC_value(S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0, lamb, mu, delta, tp):
    call_value = BCC_call_value(S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0, lamb, mu, delta)
    if tp == 'call':
        return call_value
    else:
        put_value = max(0, call_value + K * np.exp(-r*T) - S0)
        return put_value
...
def H93_call_value(S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0):
    ...
    call_value: float
        present value of European call option
    '''
    int_value = quad(lambda u: H93_int_func(u, S0, K, T, r, kappa_v, theta_v, 
                                            sigma_v, rho, v0), 0, np.inf, limit=250)[0]
    
    call_value = max(0, S0-np.exp(-r*T) * np.sqrt(S0*K)/np.pi*int_value)
    
    return call_value

def H93_value(S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0, tp):
    '''H93 model value'''
...
def M76_call_value(S0, K, T, r, v0, lamb, mu, delta):
    ...
    '''
    sigma = np.sqrt(v0)
    int_value = quad(lambda u: M76_int_func_sa(u, S0, K, T, r, sigma, lamb,
                                               mu, delta), 0, np.inf, limit=250)[0]
...
def BCC_int_func(u, S0, K, T, r, kappa_v, theta_v, 
    ...
    char_func_value = BCC_char_func(u-1j*0.5, T, r, kappa_v, theta_v, sigma_v, rho, 
                                    v0, lamb, mu, delta)
